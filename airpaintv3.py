...
while True:
    ...
    #img=cv2.flip(img,1)#mirror in x direction(horizontal)**
    #convert BGR to HSV
    ...
    if(len(conts)==2):
        if(pinchFlag==1):
            ...
            cv2.putText(img,"Two Fingures --> Mouse is Pressed ", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 2, 2)
            draw = True
        ...
        if(pinchFlag==0):
            pinchFlag=1
            cv2.putText(img,"One Fingure --> Mouse is Released ", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 2, 2)
    ...
    if draw == True and mouse_pos[0] > 100 and brush_color == BLACK and stamp_state == False:
        lstr = list(mouse_pos)
        pygame.draw.rect(windowSurface, brush_color, (lstr[0]-10,lstr[1]-10,20,20))
