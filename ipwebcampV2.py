...
arlimit_l=2000#lower area limit of valid blob
arlimit_h=7000#upper area limit of valid blob
...
while True:#read from camera repeatedly
    imgResp=urllib.urlopen(url)
    #np.uint8 data type --> unsigned int (0 to 255)
    ...
    #cv2.imshow('Mobile Camera',img)
        ...
        #exit(0)
    #f,o_frame=imgResp.read()   
    #o_frame=cv2.resize(o_frame,(320,240))
    color = "nill"
    centerr=None
    centerg=None
    frame=cv2.flip(img,1)#mirror in x direction(horizontal)**
