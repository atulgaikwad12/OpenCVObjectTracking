...
def draw_path(point_2,color):
    ...
    if point_2==None:#return if no blob is detected
     ...
     if color=="blue":
        ...
        window.set_at(pointr_2,(255,255,255))
        lstr = list(pointr_2)
        pygame.draw.rect(window,(255,255,255), (lstr[0],lstr[1],50,50))
        #pointr_1=pointr_2                 
...
while(True):#read from camera repeatedly
    ...
    if(centerg!=None):
     draw_path(centerg,"green")
    if(centerr!=None):
     draw_path(centerr,"blue")
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.VIDEORESIZE:
            ...
            for i in xrange(len(g_list)):
                 if(g_list[i]!=None):
                   draw_path(g_list[i],"green")
                 if(r_list[i]!=None):
                   draw_path(r_list[i],"blue")   
