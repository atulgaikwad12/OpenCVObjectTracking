...
bindcolor=(255,140,0)#boundary highlight color [R,G,B]
...
arlimit_l=100#lower area limit of valid blob
arlimit_h=8000#upper area limit of valid blob
...
def draw_path(point_2,color):
  ...
  if point_2==None or color == "nill":#return if no blob is detected
     ...
     if color == "green":
        ...
        cv2.circle(frame,pointg_2,3,(0,255,0),-1)
         
     if color == "blue":
        pointr_2=point_2
        lstr = list(pointr_2)
        pygame.draw.rect(window,(255,255,255), (lstr[0]-10,lstr[1]-10,20,20))
        cv2.circle(frame,pointr_2,3,(255,255,255),-1)                 
   
     #pygame.display.flip()#update pygame display window
     pygame.display.update()
     return

def remove(pixel,glist):
    lstp = []
    lstg = []
    
    #glist= [None] * 308000
    #glist = copy.copy(glistold)
    j=0
    
    for j in xrange(len(glist)):
       if(j>=0 and j<len(glist)):
         if(glist[j]!=None):
            if((cmp(glist[j],pixel))==0):
                ...
                lstg = list(glist[j])
                if(lstg[0]!=None):
                   dx=abs(lstp[0]-lstg[0])
                   if(dx>=0 and dx<=10): 
                      if(j>0 and j<len(glist)):
                        glist.pop(j)   
                        j=j-1
                if(lstg[1]!=None): 
                   dy=abs(lstp[1]-lstg[1])
                   if(dy>=0 and dy<=10):
                      if(j>0 and j<len(glist)):
                        glist.pop(j)   
                        j=j-1
...
while(True):#read from camera repeatedly
    ...
    if(centerr!=None):      
      ...
      #cv2.circle(frameg,centerr,3,(255,255,255),-1)

    for i in xrange(len(g_list)):
                 if(g_list[i]!=None):
                   cv2.circle(frameg,g_list[i],3,(0,255,0),-1)
                 if(r_list[i]!=None):
                   pixel=r_list[i]
                   n_list=remove(pixel,g_list)
                   g_list = list(n_list)
                   break
